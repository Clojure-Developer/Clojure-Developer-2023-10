...
(def thread-1
  (Thread. ^Runnable
           (fn []             ;; функция не должна принимать аргументы
             ;; тело функции выполнится в отдельном потоке
             (println 1 2 3))))
...
;; блокируем основной поток с помощью метода .join
(def thread-3
  (Thread. ^Runnable
           (fn []
             (Thread/sleep 100)
...
(defn philosopher [name left-fork right-fork]
  (Thread. ^Runnable
           (fn []
             (while true
...
(def philosophers
  (map-indexed (fn [i _]
                 (philosopher (str "Philosopher " i)
                              (nth forks i)
                              (nth forks (mod (inc i) 5))))
               forks))

(comment
 ;; Deadlock всё ещё возможен
...
;; безопасно читаем значение потока
(when (not (future-cancelled? worker))
  @worker)
...
;; используется для отложенного выполнения кода (отложенной инициализации)
...
(def counter-with-opts
  (agent 0
         ...
         :error-handler (fn [agent error]
                          (println "Agent name" (:name (meta agent)))
                          (println error))
