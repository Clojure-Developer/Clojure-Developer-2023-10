...
(s/valid? :order/date-alias #inst "2022")

(s/def :deck/suit #{:club :diamond :heart :spade})
(s/valid? :deck/suit :club)

;; Т.к. спека использует глобальный атом для хранения своих значений,
;; именованные спеки всягда должны использовать квалифицированные ключи
;; (ключ с пространством имён) для минимизации числа возможных коллизий.
(s/def :date inst?)
(s/def ::date inst?)
...
(s/conform :num/big-even :foo)
(s/conform :num/big-even 10)
(s/conform :num/big-even 100000)
...
;; Валидация мап с неквалифицированными ключами.
...
;; Фактически проверяется наличие ключа в мапе.
(s/def :example/map
...
(s/conform :ex/config ["-server" "foo"
                       "-verbose" true
                       "-user" "joe"])
