...
(comment
  (-tabulate (_map string/upper-case (_char #{\a \b \c}))
             "a" "abc" "b" "x" "")
  )
...
(defn +star
  ...
  [parser]
  (+or (+seqf cons parser (delay (+star parser)))
       (_empty ())))
